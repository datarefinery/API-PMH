...
## What do you mean 'API standards'
That's an excellent questions which I usually answer jovially with "the great thing about standards are there are so many of them".. not terribly helpful.

So when I talk about 'standards' (quotes intended) I'm actually talking about best practice, and when I say 'best practice' I really mean stuff I've decided is best practice based on desk research and in use testing.

The primary driver for my coalesced 'best practice' are, in order, ease of use, ease of development, machine efficeint.
...
* in the Request. Accept any of header content type (text/xml), extension `http://<host>/<entity>/<id>.xml`, or format=xml parameter

* in the Response. Always return data with correctly set content type in http header e.g. t 

## Paging, Sequencing, Next/Previous etc
The most important part of any harvesting API is its ability to allow harvesting in 'bites' (or in blocks or pages..). OAI-PMH implements this through the 'List' verbs in conjunction with 'resumptionToken' (this usually requires the establishment of a server side session).

The most commonly observed pattern in 'modern' APIs is to use 'URI links' to the next page, and sometimes to previous page, and even total pages. These can be implemented in http responses in the http headers and/or in the JSON return packet. Obviously the implementation is responsible generation these automatically (taking into account 'limit=' parameters for the size limit of records in a page).
